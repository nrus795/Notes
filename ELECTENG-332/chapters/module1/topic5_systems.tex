...
\end{figure}
\textcolor{red}{\textbf{Principle of Homogeneity or Scaling:}}
\begin{enumerate}[label=\blacktriangleright, leftmargin=*, itemsep=0.5em]
...
\begin{figure}[H]
    ...
    \begin{mdframed}
        \begin{center}
            \begin{tikzpicture}[auto, thick, node distance=2cm, >=Stealth]
                % Nodes
                \node [draw, rectangle, minimum height=2em, minimum width=3em, fill=white, text height=1.5ex, text depth=.25ex, line width=0.8pt, draw=blue] (system1) {\large \(\mathcal{S}\)};
                \node [draw, rectangle, minimum height=2em, minimum width=3em, fill=white, text height=1.5ex, text depth=.25ex, line width=0.8pt, draw=blue, right=5cm of system1] (system2) {\large \(\mathcal{S}\)};
                \node [left=1.5cm of system1] (input1) {\large \(x(t)\)};
                \node [right=1.5cm of system1] (output1) {\large \(y(t)\)};
                \node [left=1.5cm of system2] (input2) {\large \(ax(t)\)};
                \node [right=1.5cm of system2] (output2) {\large \(ay(t)\)};
                \node [above=0.5cm of system1, xshift=2.5cm] (homogeneity) {\large Homogeneity};
                \node [above=0.5cm of system1, xshift=4.25cm] (arrow) {\Huge \(\Rightarrow\)};
