...
Since in an armature controlled DC motor, the field current is kept constant, eqn(@eq-armature-controlled-dc-motor-torque) can be written as:
...
$$
	R_a i_a + L_a \dv{i_a}{t} + e_b = e_a \qqtext{where} e_b = K_b \dv{\theta}{t}
$${#eq-armature-controlled-dc-motor-electrical-dynamics}

The mechanical dynamics of the DC motor is given by

$$
	J \dv[2]{\theta}{t} + f_0 \dv{\theta}{t} + T_d = T_m \qqtext{where} T_m = K_T i_a
$${#eq-armature-controlled-dc-motor-mechanical-dynamics}

Taking the Laplace transform of eqn(@eq-armature-controlled-dc-motor-torque-constant), eqn(@eq-armature-controlled-dc-motor-back-emf), eqn(@eq-armature-controlled-dc-motor-electrical-dynamics) and eqn(@eq-armature-controlled-dc-motor-mechanical-dynamics) with the assumption of zero initial conditions gives

$$
\begin{aligned}
	\mathcal{L}\left[T_m \right] = K_T \mathcal{L}\left[ i_a \right] &\implies T_m(s) = K_TI_a(s) \\
	\mathcal{L}\left[e_b \right] = K_b \mathcal{L}\left[\dv{\theta}{t} \right] &\implies E_b(s) = K_bs \theta(s) \\
	\mathcal{L}\left[R_ai_a \right] + \mathcal{L}\left[L_a\dv{i_a}{t}\right] + \mathcal{L}\left[e_b\right] = \mathcal{L}\left[e_a\right] &\implies (L_as + R_a)I_a(s) = E_a(s) - E_b(s) \\
	\mathcal{L}\left[J\dv[2]{\theta}{t}\right] + \mathcal{L}\left[f_0 \dv{\theta}{t}\right] + \mathcal{L}\left[T_d\right] = \mathcal{L}\left[T_m\right] &\implies (Js^2+f_0s)\theta(s) = T_m(s) - T_d(s)
\end{aligned}
...
If the armature circuit inductance $L_a$ is neglected, then the transfer function in eqn(@eq-armature-controlled-dc-motor-transfer-function) simplifies to
...
Then the transfer function in eqn(@eq-armature-controlled-dc-motor-transfer-function-no-inductance) reduces to

$$
	\dfrac{\theta(s)}{E_a(s)} = \dfrac{\dfrac{K_T}{R_a}}{s(Js + f)}
$${#eq-armature-controlled-dc-motor-transfer-function-effective-friction}

The transfer function in eqn(@eq-armature-controlled-dc-motor-transfer-function-effective-friction) can be expressed in the form
...
![](images/feedback-block-diagram-from-notes.png)
...
\text{or,} \quad C(s)[1+G_a(s)G_b(s)H(s)] &= G_a(s)G_b(s)R(s) + G_b(s)N(s)
\end{aligned}
$$
...
Let us introduce another feedback loop through a negative feedback gain $F(s)$ as shown. Then

![](images/feedback-block-diagram-with-2-loop.png)
...
1. Closed loop control systems are [more accurate even in the presence of non-linearity]{color=red}.

2. Highly accurate as any error arising is corrected due to the presence of the feedback signal.

3. [Bandwidth range is large]{color=blue}.

4. Facilitates automation. (See the Industrial Automation section of COMPSYS-303)
...
1. They are more expensive, more complex, complicated and harder to design and hence, require more maintenance.

2. Feedback can lead to an unwanted oscillatory response.

3. Overall gain is reduced due to the presence of feedback.

4. [Stability is the most major problem]{color=red} and more care is needed to design a stable closed loop system.
